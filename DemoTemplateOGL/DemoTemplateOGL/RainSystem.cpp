...
void RainSystem::initGL() {
    ...
    glBufferData(GL_ARRAY_BUFFER, drops.size() * sizeof(RainDrop), nullptr, GL_STREAM_DRAW);
    ...
    glBufferData(GL_ARRAY_BUFFER, maxDrops * sizeof(Splash), nullptr, GL_STREAM_DRAW);
...
void RainSystem::update(Terreno *t) {
    ...
    // Upload drops to GPU (optimized mapping)
    glBindBuffer(GL_ARRAY_BUFFER, dropVBO);
    RainDrop* gpuPtr = (RainDrop*)glMapBufferRange(GL_ARRAY_BUFFER, 0, drops.size() * sizeof(RainDrop), GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);
    if (gpuPtr) {
        memcpy(gpuPtr, drops.data(), drops.size() * sizeof(RainDrop));
    ...
    if (!splashes.empty()) {
        // Upload splashes to GPU (optimized mapping)
        glBindBuffer(GL_ARRAY_BUFFER, splashVBO);
        Splash* gpuPtr = (Splash*)glMapBufferRange(GL_ARRAY_BUFFER, 0, splashes.size() * sizeof(Splash), GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);
        if (gpuPtr) {
            memcpy(gpuPtr, splashes.data(), splashes.size() * sizeof(Splash));
            glUnmapBuffer(GL_ARRAY_BUFFER);
        }
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    }
...
void RainSystem::render() {
    ...
    glUniform3fv(glGetUniformLocation(dropShader->ID, "cameraPos"), 1, &cam->getPosition()[0]);
