...
int startGameEngine(void* ptrMsg) {
    ...
    // Posición inicial deseada del jugador
    translate = glm::vec3(136.0f, 23.5f, 158.0f);
    Camera* camera = Camera::getInstance();
    Model* model = new Principal("models/personajeP/Idle.fbx", translate, camera);
    model->setTranslate(&translate);
    camera->setFront(v);
    camera->setCharacterHeight(4.0);
    scale = glm::vec3(0.01f, 0.01f, 0.01f); // escala del personaje
    ...
    // Crear la escena y fijar la posición del jugador
    ...
    n.m_halfHeight = Height * 520;
    n.m_halfDepth = Depth * 50;

    model->getModelAttributes()->at(0).hitbox = CollitionBox::GenerateAABB(translate, n, camera);

    model->setScale(&scale);
    try {
        std::vector<Animation> animations = Animation::loadAllAnimations("models/personajeP/Idle.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::vector<Animation> animation_walking = Animation::loadAllAnimations("models/personajeP/Walking.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::vector<Animation> animation_walkingB = Animation::loadAllAnimations("models/personajeP/Walking_Backwards.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::vector<Animation> animation_jump = Animation::loadAllAnimations("models/personajeP/Jumping.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::vector<Animation> animation_leftWalk = Animation::loadAllAnimations("models/personajeP/LeftWalking.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::vector<Animation> animation_rightWalk = Animation::loadAllAnimations("models/personajeP/RightWalking.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::move(animation_walking.begin(), animation_walking.end(), std::back_inserter(animations));
        std::move(animation_walkingB.begin(), animation_walkingB.end(), std::back_inserter(animations));
        std::move(animation_jump.begin(), animation_jump.end(), std::back_inserter(animations));
        std::move(animation_leftWalk.begin(), animation_leftWalk.end(), std::back_inserter(animations));
        std::move(animation_rightWalk.begin(), animation_rightWalk.end(), std::back_inserter(animations));
    ...
    // Volver a crear la escena (si es necesario) y fijar nuevamente la posición deseada
    OGLobj = new Scenario(model);
    translate = glm::vec3(136.0f, 23.5f, 158.0f);
    model->setTranslate(&translate);
    model->setNextTranslate(&translate);
...
bool checkInput(GameActions * actions, Scene * scene) {
    ...
    //ZITOS
    // Determinar si el modelo controlado es el auto
    bool isCar = (playerModel->name == "AutoChido");
    if (isCar) {
        // Deshabilitar A, D y Space cuando el auto es el modelo principal
        actions->sideAdvance = 0; // A/D
        actions->hAdvance = 0;    // movimiento horizontal alterno
        if (actions->jump) *actions->jump = 0; // Space
    }
    ...
    // Rotación de cámara por gamepad (stick derecho) - yaw/pitch
    if (actions->getAngle() != NULL) {
        float yawDelta = (*actions->getAngle()) * (6 * gameTime.deltaTime / 100);
        playerModel->setNextRotY(playerModel->getNextRotY() + yawDelta);
    }
    if (actions->getPitch() != NULL) {
        float pitchDelta = (*actions->getPitch()) * (6 * gameTime.deltaTime / 100);
        float newPitch = playerModel->cameraDetails->getPitch() + pitchDelta;
        newPitch = glm::clamp(newPitch, -89.0f, 89.0f);
        playerModel->cameraDetails->setPitch(newPitch);
    }
	...
	// Actualizamos la animacion del personaje
    if (actions->advance > 0) {
        playerModel->setAnimation(1);
    }
    else if(actions->advance < 0){
        playerModel->setAnimation(2);
    }
    else if (*actions->jump > 0) {
		playerModel->setAnimation(3);
    }
    else if (actions->sideAdvance > 0) {
		playerModel->setAnimation(4);
    }
    else if(actions->sideAdvance < 0){
		playerModel->setAnimation(5);
    }
    else {
...
void updateGasCounter(Scene* scene, int value) {
    std::vector<Texto*>* textos = scene->getLoadedText();
    for (Texto* t : *textos) {
        if (t->name == "GasCounter") {
            WCHAR buffer[32];
            swprintf(buffer, 32, L"%d", value);
            t->initTexto(buffer); // reconstruye los billboards de cada carácter
            return;
        }
    }
}
...
int gamePadEvents(GameActions * actions) {
    ...
    if (gamPad->IsConnected()) {
        gamPad->ApplyToActions(actions); // D-Pad y botón A
        ...
        if (velocidad > 0.19 || velocidad < -0.19) {
            actions->advance = velocidad > 0 ? 1 : -1; // mantener coherencia con teclado
        }
        return 1;
    }
    else
